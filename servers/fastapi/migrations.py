...
REVISION_ASSET_DUPLICATE_GOVERNANCE = "g2e6c0d4f8b9"
REVISION_ENTERPRISE_OBJECT_STORAGE = "h4a8c2e6g0b3"
REVISION_DELIVERY_LIFECYCLE_GOVERNANCE = "i5b9d3f7a1c4"
REVISION_STORAGE_RETENTION_GOVERNANCE = "j6c0e4g8b2d5"
REVISION_STORAGE_LIFECYCLE_HISTORY = "k7d1f5h9c3e6"
REVISION_ENTERPRISE_DOCUMENT_CENTER = "l8e2g6i0d4f7"
REVISION_ENTERPRISE_DOCUMENT_CHUNKS = "m9f3h7j1e5g8"
REVISION_HEAD = REVISION_ENTERPRISE_DOCUMENT_CHUNKS
    ...
    if enterprise_tables.issubset(tables):
        if "enterprise_asset_items" in tables:
            ...
            if "duplicate_status" in asset_columns:
                return REVISION_ASSET_DUPLICATE_GOVERNANCE
        ...
        if "enterprise_presentation_comment_threads" in tables:
            return REVISION_PRESENTATION_REVIEW_COLLABORATION
        ...
        if "enterprise_bid_delivery_artifacts" in tables:
            return REVISION_BID_DELIVERY
        if "enterprise_bid_presentation_releases" in tables:
            return REVISION_BID_ASSEMBLY
        ...
        if "enterprise_template_publications" in tables:
            ...
            if "scene_version" in entry_columns:
                return REVISION_SCENE_RUNTIME
            return REVISION_TEMPLATE_PUBLICATION
        entry_columns = {
            column["name"]
            for column in inspector.get_columns("enterprise_presentation_entries")
        }
        if "creation_mode" in entry_columns:
            return REVISION_ENTERPRISE_CREATION_MODE
