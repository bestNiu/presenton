...
import { AlertTriangle, ArrowLeft, CheckCircle2, FileCheck2, RefreshCw, Search, ShieldAlert, X } from "lucide-react";
...
export default function DeliveryCenterPage() {
  ...
  const [decisionTarget, setDecisionTarget] = useState<{
    incident: DeliveryIntegrityIncidentResponse;
  ...
  const decideIncident = async () => {
    if (!workspaceId || !decisionTarget || !decisionNote.trim()) return;
    setLoading(true);
    try {
      await EnterpriseApi.updateDeliveryIntegrityIncident(workspaceId, decisionTarget.incident.id, {
        status: decisionTarget.status,
        resolution_note: decisionNote.trim(),
      });
  ...
  return <main className="min-h-screen bg-[#F7F7FA] px-5 py-8 text-[#101828] sm:px-8">
    ...
    {decisionTarget && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/35 p-4" role="dialog" aria-modal="true" aria-labelledby="incident-decision-title"><div className="w-full max-w-lg rounded-2xl bg-white p-6 shadow-xl"><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-semibold text-[#635BFF]">完整性异常处置</p><h2 id="incident-decision-title" className="mt-1 text-lg font-semibold">{decisionTarget.status === "accepted_risk" ? "接受风险" : "标记为误报"}</h2><p className="mt-2 text-sm text-[#667085]">{decisionTarget.incident.resource_title}</p></div><button type="button" onClick={() => setDecisionTarget(null)} aria-label="关闭处置表单" className="rounded-lg p-1 text-[#667085] hover:bg-[#F2F4F7]"><X className="h-5 w-5" /></button></div><div className={`mt-4 rounded-xl p-3 text-xs leading-5 ${decisionTarget.status === "accepted_risk" ? "bg-[#FFF4E5] text-[#B54708]" : "bg-[#F2F4F7] text-[#475467]"}`}>{decisionTarget.status === "accepted_risk" ? "接受风险后将恢复该交付件的业务使用，请填写审批人、依据及适用范围。" : "误报结论会进入审计日志，请说明核验方法、证据及误报原因。"}</div><label className="mt-4 block text-sm font-medium text-[#344054]">处置依据<textarea autoFocus value={decisionNote} onChange={(event) => setDecisionNote(event.target.value)} rows={5} maxLength={2000} placeholder={decisionTarget.status === "accepted_risk" ? "例如：经安全负责人复核，风险仅影响历史引用展示，同意在本次交付范围内接受……" : "例如：重新计算文件及快照哈希均一致，原异常由存储同步延迟导致……"} className="mt-2 w-full rounded-xl border border-[#D0D5DD] p-3 text-sm font-normal outline-none focus:border-[#8B7DFF]" /></label><div className="mt-1 text-right text-xs text-[#98A2B3]">{decisionNote.length}/2000</div><div className="mt-5 flex justify-end gap-2 border-t border-[#EAECF0] pt-4"><button type="button" disabled={loading} onClick={() => setDecisionTarget(null)} className="h-9 rounded-lg border border-[#D0D5DD] px-4 text-sm">取消</button><button type="button" disabled={loading || !decisionNote.trim()} onClick={() => void decideIncident()} className="h-9 rounded-lg bg-[#635BFF] px-4 text-sm font-medium text-white disabled:opacity-40">确认提交处置</button></div></div></div>}
  </main>;
}
