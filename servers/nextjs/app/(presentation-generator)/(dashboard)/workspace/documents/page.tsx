  ...
  return <><main className="min-h-screen bg-[#FBFBFD] px-5 py-8 sm:px-8">
    <div className="mx-auto max-w-[1280px]">
      <header className="flex flex-wrap items-end justify-between gap-4 border-b border-[#E8E8ED] pb-6">
        ...
        <div className="flex gap-2"><select aria-label="选择工作空间" value={workspaceId} onChange={(event) => setWorkspaceId(event.target.value)} className="h-10 rounded-lg border bg-white px-3 text-sm">{workspaces.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select><button onClick={() => void loadDocuments()} className="inline-flex h-10 items-center gap-2 rounded-lg border bg-white px-3 text-sm"><RefreshCw className="h-4 w-4" />刷新</button></div>
      </header>
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
        <section className="rounded-2xl border border-[#E3E4EA] bg-white p-5"><h2 className="font-semibold">资料库</h2>
          <form onSubmit={upload} className="mt-4 grid gap-3 rounded-xl bg-[#F8F9FC] p-4 sm:grid-cols-2">
            <input aria-label="选择企业资料文件" type="file" onChange={(event) => setFile(event.target.files?.[0] || null)} className="text-sm" />
            ...
            <div className="flex gap-2"><select aria-label="资料密级" value={confidentiality} onChange={(event) => setConfidentiality(event.target.value as ConfidentialityLevel)} className="h-10 flex-1 rounded-lg border bg-white px-3 text-sm"><option value="L1">L1</option><option value="L2">L2</option><option value="L3">L3</option><option value="L4">L4</option></select><button disabled={!file || pending === "upload"} className="inline-flex h-10 items-center gap-2 rounded-lg bg-[#635BFF] px-4 text-sm text-white disabled:opacity-50">{pending === "upload" ? <Loader2 className="h-4 w-4 animate-spin" /> : <Upload className="h-4 w-4" />}上传</button></div>
          </form>
