              ...
              {presentations.slice(0, 9).map((presentation) => (
                ...
                >
                  ...
                  <div className="mt-3 flex flex-wrap gap-2 border-t border-[#F0F1F3] pt-3">
                    <Link href={`/workspace/presentations/${encodeURIComponent(presentation.id)}/review?workspace_id=${encodeURIComponent(activeWorkspaceId)}`} className="inline-flex h-7 items-center rounded-md border border-[#D9DCE3] px-2.5 text-[11px] text-[#344054]">评审与整改</Link>
                    ...
                    {presentation.status === "frozen" && <><span className="inline-flex h-7 items-center rounded-md bg-[#ECFDF3] px-2.5 text-[11px] text-[#027A48]">已形成可审计快照</span>{canCreate && <button type="button" onClick={() => void runGovernanceAction(presentation.id, () => EnterpriseApi.reopenPresentationReview(activeWorkspaceId, presentation.id))} className="h-7 rounded-md border border-[#D9DCE3] px-2.5 text-[11px]">创建新版本</button>}{canFreeze && <><button type="button" disabled={governancePending.startsWith(`export-${presentation.id}`)} onClick={() => void exportGovernedPresentation(presentation, "pptx")} className="h-7 rounded-md bg-[#635BFF] px-2.5 text-[11px] text-white disabled:opacity-40">受控 PPTX</button><button type="button" disabled={governancePending.startsWith(`export-${presentation.id}`)} onClick={() => void exportGovernedPresentation(presentation, "pdf")} className="h-7 rounded-md border border-[#635BFF] px-2.5 text-[11px] text-[#4238CA] disabled:opacity-40">受控 PDF</button></>}</>}
                  </div>
