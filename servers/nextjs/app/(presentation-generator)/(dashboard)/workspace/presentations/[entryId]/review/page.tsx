...
export default function PresentationReviewPage() {
  ...
  const focusedSlideIndex = searchParams.get("slide_index");
  ...
  useEffect(() => {
    ...
    window.requestAnimationFrame(() => {
      document.getElementById(`review-thread-${focusedThreadId}`)?.scrollIntoView({ behavior: "smooth", block: "center" });
    });
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (governanceResult.snapshots.length >= 2) {
        setFromSnapshotId(governanceResult.snapshots[1].id);
        setToSnapshotId(governanceResult.snapshots[0].id);
  ...
  const compareSnapshots = async () => {
    if (!fromSnapshotId || !toSnapshotId || fromSnapshotId === toSnapshotId) {
      setError("请选择两个不同的冻结版本进行比较");
      return;
    }
    setComparisonPending(true);
    setError(null);
    try {
      setDiff(await EnterpriseApi.comparePresentationSnapshots(workspaceId, entryId, fromSnapshotId, toSnapshotId));
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : "冻结版本比较失败");
    } finally {
      setComparisonPending(false);
    }
  };

  const inspectSnapshot = async (snapshotId: string) => {
    setComparisonPending(true);
    setError(null);
    try {
      setEvidence(await EnterpriseApi.getPresentationSnapshotEvidence(workspaceId, entryId, snapshotId));
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : "冻结证据加载失败");
    } finally {
      setComparisonPending(false);
    }
  };
    ...
    <main className="min-h-screen bg-[#F7F7FA] px-6 py-8 text-[#101828]">
      <div className="mx-auto max-w-6xl">
        ...
        <div className="mt-5 flex flex-wrap items-end justify-between gap-4">
          ...
          <div className="flex gap-2 text-xs"><Link href={governance ? `/presentation?id=${encodeURIComponent(governance.entry.presentation_id)}&type=standard&workspace_id=${encodeURIComponent(workspaceId)}&entry_id=${encodeURIComponent(entryId)}${focusedSlideIndex === null ? "" : `&slide=${encodeURIComponent(focusedSlideIndex)}`}${focusedThreadId ? `&thread_id=${encodeURIComponent(focusedThreadId)}` : ""}` : "#"} className="rounded-full bg-[#17171B] px-3 py-2 text-white">进入画布整改</Link><span className="rounded-full bg-white px-3 py-2">待处理 {openCount}</span><span className="rounded-full bg-[#FEF2F2] px-3 py-2 text-[#B42318]">阻断 {blockingCount}</span></div>
        </div>
        ...
        <div className="mt-6 grid gap-5 lg:grid-cols-[1fr_360px]">
          <section className="space-y-3">
            ...
            {threads.map((thread) => <article id={`review-thread-${thread.id}`} key={thread.id} className={`rounded-2xl border bg-white p-5 transition ${thread.id === focusedThreadId ? "border-[#8B7DFF] ring-2 ring-[#E4E1FF]" : "border-[#E3E4EA]"}`}>
          ...
          <aside className="space-y-5">
            ...
            <div className="rounded-2xl border border-[#E3E4EA] bg-white p-5">
              <div className="flex items-center gap-2 font-semibold"><GitCompareArrows className="h-4 w-4 text-[#635BFF]" />版本时间线与差异</div>
              {(governance?.snapshots.length || 0) > 0 && <div className="mt-4 space-y-2 border-l-2 border-[#E4E1FF] pl-3">{governance?.snapshots.map((snapshot, index) => <button type="button" key={snapshot.id} disabled={comparisonPending} onClick={() => void inspectSnapshot(snapshot.id)} className="block w-full rounded-lg bg-[#F8F9FC] p-2.5 text-left text-xs transition hover:bg-[#F0EEFF] disabled:opacity-50"><span className="font-semibold text-[#344054]">V{snapshot.version_no}{index === 0 ? " · 当前冻结版" : ""}</span><span className="mt-1 block text-[#667085]">{new Date(snapshot.frozen_at).toLocaleString("zh-CN")}</span><span className="mt-1 block truncate font-mono text-[10px] text-[#98A2B3]">{snapshot.manifest_hash}</span></button>)}</div>}
              {(governance?.snapshots.length || 0) >= 2 && <div className="mt-4 grid grid-cols-[1fr_auto_1fr] items-center gap-2"><select value={fromSnapshotId} onChange={(event) => setFromSnapshotId(event.target.value)} className="h-9 min-w-0 rounded-lg border border-[#D0D5DD] bg-white px-2 text-xs">{governance?.snapshots.map((snapshot) => <option key={snapshot.id} value={snapshot.id}>V{snapshot.version_no}</option>)}</select><span className="text-xs text-[#98A2B3]">对比</span><select value={toSnapshotId} onChange={(event) => setToSnapshotId(event.target.value)} className="h-9 min-w-0 rounded-lg border border-[#D0D5DD] bg-white px-2 text-xs">{governance?.snapshots.map((snapshot) => <option key={snapshot.id} value={snapshot.id}>V{snapshot.version_no}</option>)}</select><button type="button" disabled={comparisonPending || fromSnapshotId === toSnapshotId} onClick={() => void compareSnapshots()} className="col-span-3 h-9 rounded-lg bg-[#635BFF] text-xs font-medium text-white disabled:opacity-40">{comparisonPending ? "正在比较…" : "比较所选版本"}</button></div>}
              {diff ? <div className="mt-4 text-sm text-[#475467]"><p>V{diff.from_version_no} → V{diff.to_version_no}</p><div className="mt-3 grid grid-cols-2 gap-2 text-xs"><span className="rounded-lg bg-[#ECFDF3] p-2">新增 {diff.added}</span><span className="rounded-lg bg-[#FFF4E5] p-2">修改 {diff.changed}</span><span className="rounded-lg bg-[#FEF2F2] p-2">删除 {diff.removed}</span><span className="rounded-lg bg-[#F2F4F7] p-2">未变 {diff.unchanged}</span></div>{diff.slides.some((slide) => slide.change_type !== "unchanged") && <div className="mt-4 space-y-2 border-t border-[#F0F1F3] pt-3">{diff.slides.filter((slide) => slide.change_type !== "unchanged").map((slide) => <div key={slide.slide_id} className="rounded-lg bg-[#F8F9FC] p-2.5 text-xs"><div className="flex items-center justify-between"><span>{slide.change_type === "added" ? `新增第 ${(slide.after_index ?? 0) + 1} 页` : slide.change_type === "removed" ? `删除原第 ${(slide.before_index ?? 0) + 1} 页` : `修改第 ${(slide.after_index ?? 0) + 1} 页`}</span><span className={slide.change_type === "changed" ? "text-[#B54708]" : slide.change_type === "added" ? "text-[#027A48]" : "text-[#B42318]"}>{slide.change_type}</span></div>{slide.changed_fields.length > 0 && <p className="mt-1 text-[#667085]">变化字段：{slide.changed_fields.map((field) => diffFieldLabel[field] || field).join("、")}</p>}</div>)}</div>}</div> : <p className="mt-3 text-sm text-[#667085]">至少形成两个冻结版本后可进行逐页差异比较。</p>}
            </div>
